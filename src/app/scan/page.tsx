...
export default function ScanPage() {
  ...
  const fetchWaybills = async () => {
    try {
      const res = await fetch('/api/waybills?pageSize=50');
  ...
  const handleScan = async () => {
    const actualId = selectedWaybillId || v2OrderId;
    if (!actualId || !skuCode || !actualQty) return alert('请填写完整扫描信息');
      ...
      <div className="filter-bar">
        <div className="filter-row">
          <div className="filter-group" style={{ minWidth: 200 }}>
            <label>运单</label>
            <select className="input" value={selectedWaybillId} onChange={e => setSelectedWaybillId(e.target.value)}>
              <option value="">选择已有运单</option>
              {waybills.map((w: any) => (
                <option key={w.id} value={w.id}>{w.externalCode}</option>
              ))}
            </select>
          </div>
          <div className="filter-group" style={{ minWidth: 200 }}>
            <label>或输入 V2 运单号</label>
            <input className="input" placeholder="V2 运单号" value={v2OrderId} onChange={e => setV2OrderId(e.target.value)} />
          </div>
          <div className="filter-group" style={{ minWidth: 150 }}>
            <label>SKU 编码</label>
            <input className="input" placeholder="扫描条码" value={skuCode}
              onChange={e => setSkuCode(e.target.value.toUpperCase())} onKeyDown={handleKeyDown} autoFocus />
          </div>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            {result.qcResult?.passed ? (
              <div style={{ flex: 1, padding: 16, borderRadius: 4, background: 'var(--success-light)', border: '1px solid #A9DFBF' }}>
              ...
              <div style={{ flex: 1, padding: 16, borderRadius: 4, background: 'var(--error-bg)', border: '1px solid #F5B7B1' }}>
            ...
            {result.ticket && (
              <div style={{ flex: 1, padding: 16, borderRadius: 4, background: 'var(--primary-light)', border: '1px solid #AED6F1' }}>
                <div style={{ fontWeight: 500, fontSize: 13 }}>📋 已自动创建异常工单</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}>
                  工单号: {result.ticket.ticketNo}
                </div>
                <button className="btn btn-sm" style={{ marginTop: 8 }} onClick={() => router.push(`/tickets/${result.ticket.id}`)}>
                  查看工单
                </button>
              </div>
            )}
            ...
            {result.isDuplicate && (
              <div style={{ padding: 16, borderRadius: 4, background: 'var(--warning-bg)', border: '1px solid #F9E79F' }}>
          ...
          <tbody>
            {recentScans.map((s: any) => (
              <tr key={s.id}>
                <td>{s.skuCode}</td>
                <td>
                  {s.qcResult === 'pass' ? <span className="tag tag-success">通过</span> : <span className="tag tag-error">异常</span>}
                </td>
                <td>
                ...
                <td>
                  {s.ticket ? (
                    <a href={`/tickets/${s.ticketId}`} className="btn-link">{s.ticket?.ticketNo || '查看'}</a>
                  ) : '-'}
                </td>
              </tr>
            ))}
